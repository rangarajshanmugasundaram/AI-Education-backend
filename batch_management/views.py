    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            try:
                ...
                return Response({'success': True, 'data': batch}, status=status.HTTP_201_CREATED)
...
class AllocateStudentsView(APIView):
    ...
    def post(self, request, batch_id):
        ...
        if serializer.is_valid():
            ...
            if success:
                return Response({'success': True, 'message': 'Students allocated successfully'}, status=status.HTTP_200_OK)
        return Response({'success': False, 'errors': serializer.errors}, status=status.HTTP_400_BAD_REQUEST)
...
class AllocateTrainerView(APIView):
    ...
    def patch(self, request, batch_id):
        ...
        if serializer.is_valid():
            ...
            if success:
                return Response({'success': True, 'message': 'Trainer allocated successfully'}, status=status.HTTP_200_OK)
        return Response({'success': False, 'errors': serializer.errors}, status=status.HTTP_400_BAD_REQUEST)
