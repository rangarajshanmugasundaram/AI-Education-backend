...
# 💡 DEV FLAG: Set to True to instantly bypass all 403 authorization restrictions for testing/screenshots
    ...
    def decorator(view_func):
        def wrapper(self, request, *args, **kwargs):
            ...
            if not token or token != "mock-jwt-token-from-backend-xyz123":
                return Response({"error": "Unauthorized: Invalid or missing token"},
                                status=status.HTTP_401_UNAUTHORIZED)

            # 2. Extract Identifier Header
            user_email = request.headers.get('X-User-Email')
            if not user_email:
                return Response({"error": "Bad Request: Missing X-User-Email header for authentication"},
                                status=status.HTTP_400_BAD_REQUEST)

            # 3. RBAC Assessment
            user = db.users.find_one({"email": user_email.strip().lower()})
            if not user:
                return Response({"error": "Forbidden: User account context not found"},
            ...
            if normalized_user_role not in normalized_allowed_roles:
                return Response({"error": f"Forbidden: You do not have permission ({allowed_roles})"},
                                status=status.HTTP_403_FORBIDDEN)
